        ...
        public AutoCreateOutput WriteSheetCloudDataWrapperToCsFile(StaticDataCategory dataCategory)
        {
...

            {
                ...
                try
                {
                    ...
                    var sbDeserialize = new StringBuilder();
                    ...
                    sbDeserialize.AppendLine("		public void DeserializeAll()");
                    sbDeserialize.AppendLine("		{");
                    ...
                    foreach (CloudDataWrappers cloudDataWrapper in cloudDataWrappers)
                    {
                        ...
                        pkFieldType = cloudDataWrapper.pkFieldType;
                        pkFieldName = cloudDataWrapper.pkFIeldName;
                        ...
                        switch (wrapType)
                        {
                            case "list":
                                ...
                                // 에디터에서 pk값을 따로 지정했을 때,List가 아닌 DIctionary(hash)로 데이터를 관리
                                if ((!string.IsNullOrEmpty(pkFieldType) || !string.IsNullOrWhiteSpace(pkFieldType)) && pkFieldType != "index")
                                {
                                    sbWrapper.AppendLine($"        [SerializeField] private {fulltype} list = new();");
                                    sbWrapper.AppendLine($"        private Dictionary<{pkFieldType}, {classType}> _dic = new();");
                                    sbWrapper.AppendLine($"        private Dictionary<{pkFieldType}, int> _crcCodesDic = new();");
                                    sbWrapper.AppendLine($"        private Dictionary<{pkFieldType}, UnityAction<{pkFieldType}>> _changeCallback = new();");
                                    sbWrapper.AppendLine($"        public event UnityAction OnAnyChange;");
                                    sbWrapper.AppendLine($"        public int Size => _dic?.Count ?? 0;");
                                    sbWrapper.AppendLine($"        public void ClearDirty() => IsDirty = false;");
                                    sbWrapper.AppendLine();

                                    sbWrapper.AppendLine($"        public {classType} Get({pkFieldType} key)");
                                    sbWrapper.AppendLine("        {");
                                    sbWrapper.AppendLine($"			if (_crcCodesDic[key] != _dic[key].CreateCrdCode())");
                                    sbWrapper.AppendLine($"				throw new InvalidCloudDataHashException(\"Failed to retrieve cloud data due to invalid hash.\", \"{className}\", key, JsonUtility.ToJson(_dic[key]));");
                                    sbWrapper.AppendLine($"			var res = new {classType}();");
                                    sbWrapper.AppendLine($"			res = _dic[key];");
                                    sbWrapper.AppendLine($"			return res;");
                                    sbWrapper.AppendLine("        }");
                                    sbWrapper.AppendLine();

                                    sbWrapper.AppendLine($"        public void Set({pkFieldType} key, {classType} value)");
                                    sbWrapper.AppendLine("        {");
                                    sbWrapper.AppendLine($"			var crdCode = _dic[key].CreateCrdCode();");
                                    sbWrapper.AppendLine($"			if (_crcCodesDic[key] != crdCode)");
                                    sbWrapper.AppendLine($"				throw new InvalidCloudDataHashException(\"Failed to set cloud data due to invalid hash.\", \"{className}\", JsonUtility.ToJson(_dic[key]), JsonUtility.ToJson(value));");
                                    sbWrapper.AppendLine($"			_dic[key]=value;");
                                    sbWrapper.AppendLine($"			_crcCodesDic[key]=value.CreateCrdCode();");
                                    sbWrapper.AppendLine($"			IsDirty = true;");
                                    sbWrapper.AppendLine($"			if(_changeCallback.ContainsKey(key))");
                                    sbWrapper.AppendLine($"				_changeCallback[key]?.Invoke(key);");
                                    ...
                                    sbWrapper.AppendLine("        	OnAnyChange -= action;");
                                    sbWrapper.AppendLine("        }");
                                    sbWrapper.AppendLine($"        public void AddChangeListener({pkFieldType} key, UnityAction<{pkFieldType}> action)");
                                    sbWrapper.AppendLine("        {");
                                    sbWrapper.AppendLine("        	if (!_changeCallback.ContainsKey(key))");
                                    sbWrapper.AppendLine("        		_changeCallback.Add(key, action);");
                                    sbWrapper.AppendLine("        	else");
                                    sbWrapper.AppendLine("        		_changeCallback[key] += action;");
                                    sbWrapper.AppendLine("        }");
                                    sbWrapper.AppendLine($"        public void RemoveChangeListener({pkFieldType} key, UnityAction<{pkFieldType}> action)");
                                    sbWrapper.AppendLine("        {");
                                    sbWrapper.AppendLine("        	if (_changeCallback.ContainsKey(key))");
                                    sbWrapper.AppendLine("        		_changeCallback[key] -= action;");
                                    sbWrapper.AppendLine("        }");
                                    ...
                                    sbWrapper.AppendLine("		{");
                                    /*sbWrapper.AppendLine("		    foreach(var item in list)");
                                    sbWrapper.AppendLine("		    {");
                                    sbWrapper.AppendLine($"		        if(_dic.ContainsKey(item.{pkFieldName}))");
                                    sbWrapper.AppendLine("		        {");
                                    sbWrapper.AppendLine($"		            _dic[item.{pkFieldName}] = item;");
                                    sbWrapper.AppendLine("		        }");
                                    sbWrapper.AppendLine("		        else");
                                    sbWrapper.AppendLine("		        {");
                                    sbWrapper.AppendLine($"		            _dic.Add(item.{pkFieldName}, item);");
                                    sbWrapper.AppendLine("		        }");
                                    sbWrapper.AppendLine("		    }");*/
                                    sbWrapper.AppendLine("		    foreach(var kv in _dic)");
                                    sbWrapper.AppendLine("		    {");
                                    sbWrapper.AppendLine("		        var code = kv.Value.CreateCrdCode();");
                                    sbWrapper.AppendLine("		        if(_crcCodesDic.ContainsKey(kv.Key))");
                                    sbWrapper.AppendLine("		        {");
                                    sbWrapper.AppendLine("		            _crcCodesDic[kv.Key] = code;");
                                    sbWrapper.AppendLine("		        }");
                                    sbWrapper.AppendLine("		        else");
                                    sbWrapper.AppendLine("		        {");
                                    sbWrapper.AppendLine("		            _crcCodesDic.Add(kv.Key, code);");
                                    sbWrapper.AppendLine("		        }");
                                    sbWrapper.AppendLine("		    }");
                                    sbWrapper.AppendLine("		}");
                                    sbWrapper.AppendLine();

                                    sbWrapper.AppendLine($"		public void Add({pkFieldType} key, {classType} value)");
                                    sbWrapper.AppendLine("		{");
                                    sbWrapper.AppendLine("			_dic.Add(key, value);");
                                    sbWrapper.AppendLine("			_crcCodesDic.Add(key, value.CreateCrdCode());");
                                    sbWrapper.AppendLine("			IsDirty = true;");
                                    sbWrapper.AppendLine("		}");
                                    sbWrapper.AppendLine();
                                    
                                    sbWrapper.AppendLine($"		public void Remove({pkFieldType} key)");
                                    sbWrapper.AppendLine("		{");
                                    sbWrapper.AppendLine("			if(!_dic.ContainsKey(key))");
                                    sbWrapper.AppendLine("			    return;");
                                    sbWrapper.AppendLine("			_dic.Remove(key);;");
                                    sbWrapper.AppendLine("			_crcCodesDic.Remove(key);");
                                    sbWrapper.AppendLine("			IsDirty = true;");
                                    sbWrapper.AppendLine("		}");
                                    sbWrapper.AppendLine();

                                    sbWrapper.AppendLine("\t\tpublic void SerializeThis()");
                                    sbWrapper.AppendLine("		{");
                                    sbWrapper.AppendLine("		    list.Clear();");
                                    sbWrapper.AppendLine("		    foreach(var kv in _dic)");
                                    sbWrapper.AppendLine("		    {");
                                    sbWrapper.AppendLine("		        list.Add(kv.Value);");
                                    sbWrapper.AppendLine("		    }");
                                    sbWrapper.AppendLine("		}");
                                    sbWrapper.AppendLine();

                                    sbWrapper.AppendLine("\t\tpublic void DeserializeThis()");
                                    sbWrapper.AppendLine("		{");
                                    sbWrapper.AppendLine("		    foreach(var item in list)");
                                    sbWrapper.AppendLine("		    {");
                                    sbWrapper.AppendLine($"		        if(_dic.ContainsKey(item.{pkFieldName}))");
                                    ...
                                    sbDeserialize.AppendLine($"\t\t\t{className}.DeserializeThis();");
                                }
                                else
                                {
                                    sbWrapper.AppendLine($"        [SerializeField] private {fulltype} list = new();");
                    ...
                    sbDeserialize.AppendLine("		}");
                    sb.AppendLine(sbDeserialize.ToString());
