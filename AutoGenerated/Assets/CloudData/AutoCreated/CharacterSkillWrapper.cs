...
{
	[Serializable]
	public class CharacterSkillWrapper : CloudDataBase
	{
        [SerializeField] private List<CharacterSkill> list = new();
        ...
        public CharacterSkill Get(int index)
        {
			if (_crcCodes[index] != list[index].CreateCrdCode())
				throw new InvalidCloudDataHashException("Failed to retrieve cloud data due to invalid hash.", "skill", index, JsonUtility.ToJson(list[index]));
			var res = new CharacterSkill();
			res = list[index];
			return res;
        }
        public void Set(int index, CharacterSkill value)
        {
			var crdCode = list[index].CreateCrdCode();
			if (_crcCodes[index] != crdCode)
				throw new InvalidCloudDataHashException("Failed to set cloud data due to invalid hash.", "skill", index, JsonUtility.ToJson(list[index]), JsonUtility.ToJson(value));
			list[index]=value;
		...
		public void Add(CharacterSkill e)
		{
			list.Add(e);
