...
{
	[Serializable]
	public class CharacterCurrencyWrapper : CloudDataBase
	{
        [SerializeField] private List<CharacterCurrency> list = new();
        ...
        public CharacterCurrency Get(int index)
        {
			if (_crcCodes[index] != list[index].CreateCrdCode())
				throw new InvalidCloudDataHashException("Failed to retrieve cloud data due to invalid hash.", "currency", index, JsonUtility.ToJson(list[index]));
			var res = new CharacterCurrency();
			res = list[index];
			return res;
        }
        public void Set(int index, CharacterCurrency value)
        {
			var crdCode = list[index].CreateCrdCode();
			if (_crcCodes[index] != crdCode)
				throw new InvalidCloudDataHashException("Failed to set cloud data due to invalid hash.", "currency", index, JsonUtility.ToJson(list[index]), JsonUtility.ToJson(value));
			list[index]=value;
		...
		public void Add(CharacterCurrency e)
		{
			list.Add(e);
