...
{
	[Serializable]
	public class CharacterTestWrapper : CloudDataBase
	{
        [SerializeField] private List<CharacterTest> list = new();
        ...
        public CharacterTest Get(int index)
        {
			if (_crcCodes[index] != list[index].CreateCrdCode())
				throw new InvalidCloudDataHashException("Failed to retrieve cloud data due to invalid hash.", "tests", index, JsonUtility.ToJson(list[index]));
			var res = new CharacterTest();
			res = list[index];
			return res;
        }
        public void Set(int index, CharacterTest value)
        {
			var crdCode = list[index].CreateCrdCode();
			if (_crcCodes[index] != crdCode)
				throw new InvalidCloudDataHashException("Failed to set cloud data due to invalid hash.", "tests", index, JsonUtility.ToJson(list[index]), JsonUtility.ToJson(value));
			list[index]=value;
		...
		public void Add(CharacterTest e)
		{
			list.Add(e);
