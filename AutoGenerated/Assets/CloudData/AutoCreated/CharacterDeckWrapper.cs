...
{
	[Serializable]
	public class CharacterDeckWrapper : CloudDataBase
	{
        [SerializeField] private List<CharacterDeck> list = new();
        ...
        public CharacterDeck Get(int index)
        {
			if (_crcCodes[index] != list[index].CreateCrdCode())
				throw new InvalidCloudDataHashException("Failed to retrieve cloud data due to invalid hash.", "deck", index, JsonUtility.ToJson(list[index]));
			var res = new CharacterDeck();
			res = list[index];
			return res;
        }
        public void Set(int index, CharacterDeck value)
        {
			var crdCode = list[index].CreateCrdCode();
			if (_crcCodes[index] != crdCode)
				throw new InvalidCloudDataHashException("Failed to set cloud data due to invalid hash.", "deck", index, JsonUtility.ToJson(list[index]), JsonUtility.ToJson(value));
			list[index]=value;
		...
		public void Add(CharacterDeck e)
		{
			list.Add(e);
